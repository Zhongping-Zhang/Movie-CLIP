...
from models.movieclip import trailer_text
from train import str2bool, sigmoid

def train(epoch):
    model.train()
    for batch_idx, sample in enumerate(train_loader):
        movie_id, label, text_tokens = sample['movie_id'],sample['label_onehot'],sample['text_tokens']
        if "text_feature" in sample:
            text_feature = sample['text_feature'].to(device).type(FloatTensor)
        else:
            text = clip.tokenize(text_tokens,context_length=77,truncate=True).to(device) # (256,77)
            text_feature = clip_model.encode_text(text).to(device).type(FloatTensor) # (256,768), ViT-L/14@336px
...
def main(args):
    ...
    log_name = os.path.join("logs", args.model_name+"_")
    ...
    traindata = trailer_multimodal_features(phase='train',
                                        ...
                                        visual_feature_file=args.visual_feature_version+"_train.pkl" if args.visual_feature_version else None,
    ...
    valdata = trailer_multimodal_features(phase='val',
                                        data_dir=args.data_dir,
                                        visual_feature_dir=args.visual_feature_dir,
                                        visual_feature_file=args.visual_feature_version+"_val.pkl" if args.visual_feature_version else None,
                                        audio_feature_file=args.audio_feature_file,
                                        text_token_file=args.text_token_file,
                                        text_feature_file=args.text_feature_file,
                                        num_classes=args.num_categories,
    ...
    model = trailer_text(text_feature_dim=768,
...
if __name__=="__main__":
    ...
    parser.add_argument('--model_name', type=str, default='trailer_text', help='model name')
    parser.add_argument('--data_dir', type=str, default='data/CondensedMovies/MovieCLIP_features',
                        help='path to save data files')
    parser.add_argument('--visual_feature_dir', type=str, default=None,
                        help='path to visual features')
    parser.add_argument('--visual_feature_version', type=str, default=None)
    ...
    if args.text_feature_file is None:
        import clip
        global clip_model
        clip_model, clip_preprocess = clip.load("ViT-L/14@336px", device=device, download_root="/projectnb/ivc-ml/zpzhang/checkpoints/clip_cache")
