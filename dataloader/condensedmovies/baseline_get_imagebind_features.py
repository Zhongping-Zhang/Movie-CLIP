...
parser.add_argument('--model_name', default="imagebind",)
parser.add_argument('--model_version', default="imagebind_default")
parser.add_argument('--data_root', default='data/CondensedMovies/MovieCLIP_features')
parser.add_argument('--cache_dir', type=str, default="/projectnb/ivc-ml/zpzhang/checkpoints/eva_clip_cache")
args = parser.parse_args()



if args.model_name=="eva_clip":
...
def get_image_imagebind_features(image_paths, normalize=False):
    inputs = {
    ...
    return embeddings['vision'].cpu().detach().numpy() # (1,1024)
...
for index in tqdm(range(len(data)), desc='extract clip features'):
    ...
    if args.model_name!="imagebind":
        ...
        shot_frame0_feats = get_image_imagebind_features([shot_frame0_path])
        shot_frame1_feats = get_image_imagebind_features([shot_frame1_path])
        shot_frame2_feats = get_image_imagebind_features([shot_frame2_path])

    # print("shot_frame0_feats:", shot_frame0_feats.shape)
