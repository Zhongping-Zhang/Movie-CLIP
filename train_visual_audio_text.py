...
from models.movieclip import trailer_visual_audio_text
from train import str2bool,sigmoid
...
def train(epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output_dict = model(visual_feature, audio_feature, text_feature)
        visual_output, audio_output, text_output, output = output_dict["visual_output"],\
                                                           output_dict["audio_output"], output_dict['text_output'], output_dict["multimodal_output"]

        visual_loss = F.binary_cross_entropy_with_logits(visual_output, label.float())
        audio_loss = F.binary_cross_entropy_with_logits(audio_output, label.float())
        text_loss = F.binary_cross_entropy_with_logits(text_output, label.float())
        loss = F.binary_cross_entropy_with_logits(output, label.float())

        # print("alpha,beta:",model.alpha,model.beta)
        if args.include_modality_loss is True:
            loss+=visual_loss+audio_loss+text_loss
        loss.backward()
        optimizer.step()
...
def main(args):
    ...
    audio_feature_dim = test_sample['audio_feature'].shape[0]
    text_feature_dim = test_sample['text_feature'].shape[0]

    model = trailer_visual_audio_text(visual_feature_dim=visual_feature_dim,
                           ...
                           trainable_composition=args.trainable_composition,
                           alpha=args.alpha,
                           beta=args.beta,
                           ).to(device)
    ...
    for epoch_org in range(args.num_epoch):
        epoch = epoch_org + 1
        train(epoch)
        print("model alpha/beta",model.alpha,model.beta)
...
if __name__=="__main__":
    ...
    parser.add_argument('--visual_feature_version', type=str, default="ViT-L-14@336px")
    parser.add_argument('--audio_feature_file', type=str,
                        default="data/CondensedMovies/MovieCLIP_features/Audio_features/PANNs_embeddings_all.pkl")
    parser.add_argument('--text_token_file',type=str,
                        default="data/CondensedMovies/MovieCLIP_features/Text_features/whisper_medium_asr_output.json")
    parser.add_argument('--text_feature_file',type=str,
                        default="data/CondensedMovies/MovieCLIP_features/Text_features/whisper_medium_asr_output_ViT-L-14@336px.pkl")
    ...
    parser.add_argument('--include_modality_loss', type=str2bool, default=False,
                        help="whether to include the loss for each modality")
    parser.add_argument('--trainable_composition',type=str2bool,default=False)
    parser.add_argument('--alpha',type=float,default=0.5,help="initialize the parameter to control the weight of visual modality")
